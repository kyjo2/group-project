...
void	delete_env2(char *line, t_env *change_env)
{
...
void	delete_env1(char *line, t_env *change_env) // $? 처리 $부터 시작
{
	int	i;
	int	j;
...
void	ft_change_env(char *line, t_env *change_env, int i, int doubleq_flag)
{
	...
	if (env_flag == 0 && doubleq_flag == 0)
		delete_env1(line[--i], change_env); // $부분부터
	else if (env_flag == 0 && doubleq_flag == 1)
		delete_env2(line[--i], change_env);
}
...
t_list	*make_node(char *line, int pipe_flag, char **envp, t_env *change_env)
{
	...
	final_change_line(line ,  );  // 여기서 " " 랑 '' 이것들 다 없애준다!
